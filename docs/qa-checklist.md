...
### TIER 테이블 분리
- [ ] TIER 테이블에 UNCLASSIFIED 소재 미포함
- [ ] TIER1~4 + LOW_VOLUME만 TIER 테이블에 표시

### 신규 소재 섹션
- [ ] 집행일수 3일 미만 소재만 "신규 소재 (평가 중)" 섹션에 표시
- [ ] 신규 소재가 있을 때만 섹션 표시 (없으면 숨김)
- [ ] 컬럼: 소재명, 지점, 비용, 전환, CPA, CTR, CVR, 집행일수

### 지점별 효율점수
- [ ] 지점별 테이블에 "효율점수" 컬럼 표시
- [ ] 효율점수 색상 구분: ≥1.2 녹색, 0.8~1.2 파랑, <0.8 빨강
- [ ] 권고 문구 없음 (숫자만 표시)

### 인사이트 확장
- [ ] 기존 인사이트 유지 (악화/개선/변동)
- [ ] TIER1 미집행 지점 확장 기회 인사이트 추가
- [ ] 확장 기회 색상: 파랑 (#60a5fa)
